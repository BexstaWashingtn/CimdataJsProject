...
export async function init() {
  ...
  el("#channels").addEventListener("mousedown", (ev) => {
    ...
    channelController.currentChannel =
      parseInt(ev.target.closest(".channel").getAttribute("data-id")) - 1;

    if (
      ev.target.nodeName === "BUTTON" &&
      ev.target.classList.contains("channelNumber")
    ) {
  ...
  window.addEventListener("changeChannel", (e) => {
    ...
    sequencer.stepNodes.forEach((step, index) => {
      toggleClass(
        step,
        "selected",
        !!stepStates[channelController.currentChannel][index],
      );
    });
  });
  ...
  window.addEventListener("timerStopped", () => {
    if (
      sequencer.currentStep - 1 > 0 &&
      sequencer.currentStep - 1 < sequencer.maxSteps
    ) {
